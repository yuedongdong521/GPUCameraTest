...
- (void)initToolBar {
  ...
  UIBarButtonItem* item2 =
      [[UIBarButtonItem alloc] initWithTitle:@"美颜"
                                       style:UIBarButtonItemStylePlain
                                      target:self
                                      action:@selector(toolbarAction:)];
  [item2 setTag:2];
  UIBarButtonItem* emptyItem = [[UIBarButtonItem alloc]
...
- (void)rightAction {
  ...
  [self saveVideo:[[NSBundle mainBundle] pathForResource:@"Movie"
                                                  ofType:@"mp4"]];
}
...
- (void)trimVideoForUrl:(NSURL*)videoUrl {
  ...
  int64_t leftTime =
      ((viewOriginX(_leftImageView) + viewWidth(_leftImageView)) /
       (viewWidth(_bgImageView) - 40)) *
      CMTimeGetSeconds(videoDuration) * 1000;
  CMTime startTime = CMTimeMake(leftTime, 1000);

  int64_t rightTime =
      (viewOriginX(_rightImageView) / (viewWidth(_bgImageView) - 40)) *
      CMTimeGetSeconds(videoDuration) * 1000;
  CMTime endTime = CMTimeMake(rightTime, 1000);
  NSLog(@"duration = %f", CMTimeGetSeconds(CMTimeSubtract(endTime, startTime)));
  __weak typeof(self) weakSelf = self;
  [self.videoEnditor
      trimVideoForFileUrl:videoUrl
                StartTime:startTime
                   EndStr:endTime
          BackNewVideoUrl:^(NSURL* url) {
            AVAsset* asset = [AVAsset assetWithURL:url];

            NSLog(@"编辑后的视频时长 %f", CMTimeGetSeconds(asset.duration));
            [weakSelf resetPlayerWidthURL:url];
            [weakSelf saveVideo:url.absoluteString];
          }];
}
...
- (void)resetPlayerWidthURL:(NSURL *)url
{
  AVPlayerItem* newItem =
  [AVPlayerItem playerItemWithURL:url];
  [player replaceCurrentItemWithPlayerItem:newItem];
  [self resetGPUMoivePlayWithPlayerItem:newItem];
  [self startPlayer];
}

- (void)resetGPUMoivePlayWithPlayerItem:(AVPlayerItem*)playerItem {
  return;
  if (gpuMovie) {
  ...
  gpuMovie.shouldRepeat = NO;
  gpuMovie.playAtActualSpeed = YES;
  [gpuMovie addTarget:_videoView];
}

- (void)initAVPlayer {
  UIImageView* imageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(50, 84, ScreenWidth - 100, 216)];
...
//  _videoView = [[GPUImageView alloc] initWithFrame:imageView.bounds];
//  [imageView addSubview:_videoView];
  ...
  [self resetGPUMoivePlayWithPlayerItem:playerItem];
  ...
  _progressLabel = [[UILabel alloc]
      initWithFrame:CGRectMake(viewOriginX(progressBgView) +
                                   viewWidth(progressBgView) + 10,
                               viewOriginY(progressBgView), 60, 50)];
  ...
  __weak typeof(self) weakself = self;
  _playerTimeObserver = [player
      addPeriodicTimeObserverForInterval:CMTimeMake(1, 10)
                                   queue:dispatch_get_main_queue()
                              usingBlock:^(CMTime time) {
                                if (weakself.isPlaying) {
                                  [weakself
                                      changeProgressLabelValueForTime:time
                                                 ForVideoDruationTime:
                                                     CMTimeGetSeconds(
                                                         urlAsset.duration)];
                                }
                              }];
...
- (void)changeVideoSpeedAction:(UIButton*)btn {
  ...
  __weak typeof(self) weakSelf = self;
  [self.videoEnditor changeVideoSpeedForUrl:self.videoUrl
                              ForSpeedScale:speedValue
                         ForBackNewVideoUrl:^(NSURL* newUrl) {
                           if (newUrl) {
                             [weakSelf resetPlayerWidthURL:newUrl];
                           }
                         }];
}
...
- (void)initEnditorVideoView {
  ...
  for (int i = 0; i < array.count; i++) {
    UIImageView* imageView = [[UIImageView alloc]
        initWithFrame:CGRectMake(20 + width * i, 0, width, viewHeight(bgView))];
  ...
  UIImageView* leftImageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, 0, 30, viewHeight(bgView))];
  UIImage* leftImage = [UIImage imageNamed:@"left@2x"];

  leftImageView.image =
      [leftImage resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)
  ...
  UIImageView* rightImageView =
      [[UIImageView alloc] initWithFrame:CGRectMake(viewWidth(bgView) - 30, 0,
                                                    30, viewHeight(bgView))];
  UIImage* rightImage = [UIImage imageNamed:@"right@2x"];

  rightImageView.image =
      [rightImage resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)
                                 resizingMode:UIImageResizingModeStretch];
  ...
  UIPanGestureRecognizer* rightPan = [[UIPanGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(rightPanAction:)];
  ...
  _topBgView = [[UIView alloc]
      initWithFrame:CGRectMake(30, 0, viewWidth(bgView) - 60, 2)];
  _topBgView.backgroundColor = [UIColor colorWithRed:13 / 255.0
                                               green:197 / 255.0
                                                blue:162 / 255.0
                                               alpha:1.0];
  ...
  _bottomBgView =
      [[UIView alloc] initWithFrame:CGRectMake(30, viewHeight(bgView) - 2,
                                               viewWidth(bgView) - 60, 2)];
  _bottomBgView.backgroundColor = [UIColor colorWithRed:13 / 255.0
                                                  green:197 / 255.0
                                                   blue:162 / 255.0
                                                  alpha:1.0];
  [_bgImageView addSubview:_bottomBgView];

  UISlider* speedSilder =
      [[UISlider alloc] initWithFrame:CGRectMake(50,
                                                 viewOriginY(_bgImageView) +
                                                     viewHeight(_bgImageView),
                                                 150, 50)];
  ...
  [speedSilder addTarget:self
                  action:@selector(changeVideoSpeed:)
        forControlEvents:UIControlEventValueChanged];
  [_enditorBgView addSubview:speedSilder];

  _speedLabel =
      [[UILabel alloc] initWithFrame:CGRectMake(210,
                                                viewOriginY(_bgImageView) +
                                                    viewHeight(_bgImageView),
                                                50, 50)];
  ...
  speedBtn.frame = CGRectMake(
      270, viewOriginY(_bgImageView) + viewHeight(_bgImageView), 50, 50);
  [speedBtn setTitle:@"确定" forState:UIControlStateNormal];
  [speedBtn addTarget:self
                action:@selector(changeVideoSpeedAction:)
      forControlEvents:UIControlEventTouchUpInside];
...
- (void)rightPanAction:(UIGestureRecognizer*)pan {
  ...
  if (viewWidth(_rightImageView) - _startRightPt.x + pt.x >=
      viewWidth(_bgImageView) - viewWidth(_rightImageView) * 0.5) {
    rightX = viewWidth(_bgImageView) - viewWidth(_rightImageView) * 0.5;
  } else if (-_startRightPt.x + pt.x <=
             viewOriginX(_leftImageView) + viewWidth(_leftImageView)) {
    rightX = viewOriginX(_leftImageView) + viewWidth(_leftImageView) +
             viewWidth(_rightImageView) * 0.5;
  } else {
    rightX = viewWidth(_rightImageView) - _startRightPt.x + pt.x;
  }
  _rightImageView.center = CGPointMake(rightX, rightY);
  _topBgView.frame =
      CGRectMake(viewOriginX(_leftImageView) + viewWidth(_leftImageView),
                 viewOriginY(_topBgView),
                 viewOriginX(_rightImageView) - viewOriginX(_leftImageView) -
                     viewWidth(_leftImageView),
                 viewHeight(_topBgView));
  _bottomBgView.frame =
      CGRectMake(viewOriginX(_leftImageView) + viewWidth(_leftImageView),
                 viewOriginY(_bottomBgView),
                 viewOriginX(_rightImageView) - viewOriginX(_leftImageView) -
                     viewWidth(_leftImageView),
                 viewHeight(_bottomBgView));

  AVAsset* asset = [AVAsset assetWithURL:self.videoUrl];
  CMTime videoDuration = asset.duration;
  int rightTime = ((viewOriginX(_rightImageView) - viewWidth(_rightImageView)) /
                   (viewWidth(_bgImageView) - 60)) *
...
- (void)letfPanAction:(UIGestureRecognizer*)pan {
  ...
  if (viewWidth(_leftImageView) - _startLeftPt.x + pt.x >=
      viewOriginX(_rightImageView) - viewWidth(_leftImageView) * 0.5) {
  ...
  CMTime videoDuration = asset.duration;
  int leftTime =
      (viewOriginX(_leftImageView) / (viewWidth(_bgImageView) - 60)) *
      CMTimeGetSeconds(videoDuration);
...
- (void)progressValueChange:(UIGestureRecognizer*)pan {
  ...
  if (value <= 0) {
    ...
    timeValue = value / viewWidth(_progressView) *
                CMTimeGetSeconds(player.currentItem.duration);
  }

  NSLog(@"进度条value = %f 当前时间 Time= %f", _progressView.progress,
        timeValue);
  ...
  if (self.isPlaying) {
    ...
    [self changeProgressLabelValueForTime:currentTime
                     ForVideoDruationTime:CMTimeGetSeconds(
                                              player.currentItem.duration)];
  }
}

- (void)changeProgressLabelValueForTime:(CMTime)time
                   ForVideoDruationTime:(CGFloat)druationTime {
  [self.progressView setProgress:CMTimeGetSeconds(time) / druationTime
                        animated:YES];
  int currentTime = (int)CMTimeGetSeconds(time);
  self.progressLabel.text =
      [NSString stringWithFormat:@"%d/%ds", currentTime, (int)druationTime];
...
- (UIImage*)getImageForVideo:(NSURL*)videoURL atTime:(NSTimeInterval)time {
  ...
  NSLog(@"视频画面当前时间帧：%f",
        CMTimeGetSeconds(CMTimeMake(thumbnailImageTime, 60)));
